...
def main():
    ...
    fname = '/proj/ltu_mtl/users/x_ricup/results/' + p['dataset_name'] + '/' + p['Experiment_name']
    ...
    model = model.cuda()
    # model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[0,1], output_device=2)


    ## to check the model parameters 
    # input_size = (3,256,256)
    # print(summary(model, input_size, batch_size=-1, device='cuda') )
    ...
    # prof = torch.profiler.profile(
                    # schedule=torch.profiler.schedule(wait=p['profiler']['wait'], warmup=p['profiler']['warmup'], active=p['profiler']['active'], repeat=p['profiler']['repeat']),on_trace_ready=torch.profiler.tensorboard_trace_handler('../runs/'+ p['Experiment_name']),record_shapes=True, with_stack=True)
