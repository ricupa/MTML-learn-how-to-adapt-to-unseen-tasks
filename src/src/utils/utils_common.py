...
def get_transformations(p):
    ...
    # transform_tr = [tr.FixedResize((256,256)), tr.ToTensor(),
    #                 tr.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]
    # transform_v = [tr.FixedResize((256,256)), tr.ToTensor(), 
    #                 tr.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]
...
def get_dataset(p,fold=None):
    ...
    if p['dataset_name'] == 'Taskonomy':
        ...
        p['data_dir'] = "/proj/ltu_mtl/dataset/taskonomy_dataset/"
        ...
        p['data_dir'] = "/proj/ltu_mtl/dataset/NYU_dataset/NYUD_MT"
    ...
    def forward(self, images, targets, new_task_list):              
        ...
        if self.set_up == 'multi_task':            
            ...
            if self.comb_loss == 'sum':
                ...
                # for i, task in enumerate(self.tasks):
                #     pre = torch.clamp(torch.exp(-self.log_vars[i]), min = 0)
                #     if i ==0:
                #         loss= pre * out_loss[task] + torch.clamp(self.log_vars[i], min = 0)                        
                #     else:
                #         loss+= pre * out_loss[task] + torch.clamp(self.log_vars[i], min = 0)                      
                # loss = torch.mean(loss)     # dosen't matters if there is only one value of loss      
    ...
    for task in p['task_list']:
        ...
        if (task == 'class_scene') or (task == 'class_object'):            
            ...
            if p['dataset_name'] == 'NYU':
                ...
                _, _, _ , _ , m['mae'], _ = get_depth_metric(output[task].cuda(), label[task].cuda(), binary_mask)            
                metrics[task] = m 
...
def tb_logger(p,epoch,loss,metric,writer,set):
    ...
    #         writer.add_scalar(k+"/train/loss", v, epoch)

    #     for task, v in metric.items():
    #         for m, val in v.items():
    #             writer.add_scalar(task+"/train/"+ m, val, epoch)
    # else:
    for k,v in loss.items():
